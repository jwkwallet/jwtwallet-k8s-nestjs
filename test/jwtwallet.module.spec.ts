...
describe("JWTWalletModule", () => {
  ...
  beforeEach(async () => {
    module = await Test.createTestingModule({
      imports: [ScheduleModule.forRoot(), JWTWalletModule.forRoot(testOptions)]
    }).compile();
  });
  ...
  describe("Module Configuration", () => {
    ...
    it("should configure service with provided options", () => {
      ...
      expect((service as any).keyExpirationSeconds).toBe(
        testOptions.keyExpirationSeconds
      );
      expect((service as any).keyRotationIntervalSeconds).toBe(
        testOptions.keyRotationIntervalSeconds
      );
