...
describe('JWTWalletService', () => {
  ...
  beforeEach(async () => {
    ...
    (KubeConfig as jest.MockedClass<typeof KubeConfig>).mockImplementation(() => mockKubeConfig as any);
  ...
  describe('Constructor', () => {
    ...
    it('should use default values when options are not provided', async () => {
      ...
      const serviceWithDefaults = module.get<JWTWalletService>(JWTWalletService);
    ...
    it('should use default issuer when not provided', async () => {
      ...
      const serviceWithDefaults = moduleWithoutIssuer.get<JWTWalletService>(JWTWalletService);
      
      expect((serviceWithDefaults as any).issuer).toBe('jwtwallet.default.issuer');
  ...
  describe('saveKeys', () => {
    ...
    it('should handle Kubernetes API errors', async () => {
      ...
      mockKubeClient.createNamespacedCustomObject.mockRejectedValue(kubernetesError);
      const expiresOn = Date.now() + 3600000;

      await expect((service as any).saveKeys(mockKeypair, expiresOn)).rejects.toThrow(kubernetesError);
      expect(mockLogger.error).toHaveBeenCalledWith(`Error saving key to K8s: ${kubernetesError}`);
    });
  });
  ...
  describe('rollKeys', () => {
    ...
    it('should roll keys successfully', async () => {
      ...
      expect((service as any).saveKeys).toHaveBeenCalledWith(mockKeypair, 1000000000 + 3600000);
      expect(mockLogger.log).toHaveBeenCalledWith('Rolling keys in namespace: test-namespace');
    ...
    it('should handle saveKeys errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(`Failed to save keys to K8s: ${saveError}`);
    });
  });
  ...
  describe('onModuleInit', () => {
    ...
    it('should initialize module successfully', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith('JWTWalletService initialized');
      expect(setIntervalSpy).toHaveBeenCalledWith(expect.any(Function), 300000);
      expect(mockSchedulerRegistry.addInterval).toHaveBeenCalledWith('jwtwallet-key-rotation', 123);
    });
  ...
  describe('signToken', () => {
    ...
    it('should throw PrivateKeyMissingError when no active key', async () => {
      ...
      await expect(service.signToken(payload, expiresOn)).rejects.toThrow(PrivateKeyMissingError);
  ...
  describe('getJwkByKid', () => {
    ...
    it('should handle Kubernetes API errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(`Error fetching key from K8s for kid ${kid}: ${k8sError}`);
    });
  });
  ...
  describe('verifyToken', () => {
    const mockToken = 'eyJhbGciOiJFUzI1NiIsImtpZCI6InRlc3Qta2lkLTEyMyJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.signature';
    const mockPayload = { sub: '1234567890' };
    const audience = 'test-audience';
    const mockPublicJwk = { kty: 'EC', crv: 'P-256', x: 'test-x', y: 'test-y' };

    beforeEach(() => {
      (jose.jwtVerify as jest.Mock).mockImplementation((token, keyFunction, options) => {
        // Call the key function with a mock header
        return keyFunction({ kid: 'test-kid-123' }).then((key) => {
    ...
    it('should verify token using JWK from cache/k8s when kid does not match active key', async () => {
      (service as any).activeKey = { ...mockKeypair, privateKeyKid: 'different-kid' };
    ...
    it('should throw KeyIdDidNotMatchError when no kid in header', async () => {
      ...
      await expect(service.verifyToken(mockToken, audience)).rejects.toThrow(KeyIdDidNotMatchError);
      expect(mockLogger.error).toHaveBeenCalledWith('No key ID (kid) found in token header');
    });
    ...
    it('should throw KeyMissingError when JWK is not found', async () => {
      ...
      await expect(service.verifyToken(mockToken, audience)).rejects.toThrow(KeyMissingError);
      expect(mockLogger.error).toHaveBeenCalledWith('No public key found for kid: test-kid-123');
    });
    ...
    it('should handle getJwkByKid errors', async () => {
      ...
      await expect(service.verifyToken(mockToken, audience)).rejects.toThrow(jwkError);
    });
  });
